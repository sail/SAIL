...
public class ReportConstructorPanel extends Panel
{
 ...
 private ParameterSelectPanel ptbl;
 ...
 public ReportConstructorPanel( Action[] auxButtons )
 {
  super();
  
  setAutoScroll(true);
  setTitle("Report constructor");
  setIconCls("tab-icon");
  setClosable(false);
  setAutoWidth(true);
...
//    new Action("Add Split","enum","splitIcon",null),
  ...
  ptbl = new ParameterSelectPanel(false, btns, DataManager.getInstance().getParameterManager() );
...
//  ptbl.setButtonEnabled( "enum", false, true );
  ...
  pp = new PaddedPanel(requestTree,4,2,4,4);
  pp.setLayout(new FitLayout());
  pp.setWidth(350);
  add(pp, eastData);

  studySelListeners.add( requestTree );
  ...
  requestTree.setRequestItemSelectListener( new RequestItemSelectListenerImpl() );
 }
 ...
 {
  requestTree.addReportListener(lsnr);
 }
 ...
 private class SelectParamLsnr implements ObjectAction<Parameter>
 {
  ...
  public void doAction(String actType, Parameter p)
  {
   if("add".equals(actType))
    requestTree.addParameter(p);
   else if("qquery".equals(actType))
    requestTree.quickQuery(p);
   ...
   {
    requestTree.addParameterToGroup(p);
   }
    ...
    fDlg.setListener( new ObjectAction<ComplexFilter>()
      {
       ...
       public void doAction(String actName, ComplexFilter cfp)
       {
        ...
        {
         if( toGrp )
          requestTree.addParameterToGroup(cfp.getParameter());
         else
          requestTree.addParameter(cfp.getParameter());
        }
        else
        {
         if( toGrp )
          requestTree.addFilteredToGroup(cfp);
         else
          requestTree.addFiltered(cfp);
        }
  ...
  {
   if( "add".equals(actType) )
    requestTree.addGroup(lp);
   else if("qquery".equals(actType))
    requestTree.quickQuery(lp);
   else if( "addGrp".equals(actType) )
   {
    requestTree.addParameterToGroup(lp);
   }
   else if( "exportvis".equals(actType) )
   {
    exportParameters(ptbl.getVisible());
...
//   System.out.println("OK "+fq.getQualifier().getParameter().getName());
   ...
   {
    requestTree.addSplitFiltered(fq);
   }
   else if( fq.getEnumeration() instanceof Variable )
    requestTree.addVariable((Variable)fq.getEnumeration());
   else
    requestTree.addParameter((Qualifier)fq.getEnumeration());
  }
