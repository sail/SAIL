...
//  table.setLayout( new TableLayout(len+1) );
 ...
 /*
 interface CellRendererCallBack
 {
  CellRendererInfo getCellRendererInfo(int ind, RequestItem p, int depth, int level, boolean exprFirst, boolean groupFirst);
 }
...
//  }
  ...
  private String cls,txt;
//  private RowType summary;
  private ReportRequest reqStr;

  public CellRendererInfo(String clazz, String text)
  {
   cls=clazz;
   txt=text;
...
*/
 ...
 private int createExpression( int ind, GroupRequestItem grp, StringBuilder expr )
 {
...
//  boolean topExpr = ind == 1;
  ...
  {
   int d = grp.getItems().size()==grp.getDepth()?-1:grp.getDepth();
   ...
   else
   {
    ...
    if( d != 1 && d != -1 )
     expr.append("<span class='anyOpText'>any " + grp.getDepth() + " of</span> ");
   }
   ...
   if( rri instanceof ParameterRequestItem  )
   {
    ...
    if( rri instanceof ComplexFilteredRequestItem )
    {
     ...
     String refStr = "<sup>" + ind + "</sup> - parameter with code: '" + p.getCode() + "' and name: '" + p.getName()
       + "' filtered out by: <ul class='filterDesc'>";

     List<Range> ranges = ((ComplexFilteredRequestItem) rri).getFilter().getRealRanges();

     if(ranges != null)
     {
      for(Range rg : ranges)
      {
       Collection< ? extends ParameterPart> parts = p.getAllVariables();
       ...
       if(parts != null)
       {
        for(ParameterPart pp : parts)
        {
         ...
         {
          refStr += "<li>" + pp.getName() + " within " + rg.toString() + "</li>";
          break;
         }
        }
       }
     ...
     List<IntRange> intRanges = ((ComplexFilteredRequestItem) rri).getFilter().getIntRanges();
        ...
        for(ParameterPart pp : parts)
        {
         if(pp.getId() == rg.getPartID())
         {
          if(((Variable) pp).getType() == uk.ac.ebi.sail.client.common.Variable.Type.BOOLEAN)
           refStr += "<li>'" + pp.getName() + "' is " + (rg.getLimitLow() == 0 ? "False" : "True") + "</li>";
          else if(((Variable) pp).getType() == uk.ac.ebi.sail.client.common.Variable.Type.DATE)
           refStr += "<li>'" + pp.getName() + "' is from " + Date2Str.int2DateStr(rg.getLimitLow()) + " till " + Date2Str.int2DateStr(rg.getLimitHigh());
          else
           refStr += "<li>'" + pp.getName() + "' within " + rg.toString() + "</li>";
          break;
         }
     ...
     List<List<Integer>> varis = ((ComplexFilteredRequestItem) rri).getFilter().getVariants();
     ...
     if(varis != null)
     {
      for(List<Integer> ppVari : varis)
      {
       ...
       for(int m = 0; m < 2; m++)
       {
        ...
        parts = m == 0 ? p.getAllVariables() : p.getAllQualifiers();
     ...
     refStr += "</ul>";
     references.add( new ParamRef(refStr, (ParameterRequestItem)rri));
    }
    else
     references.add(new ParamRef("<sup>" + ind + "</sup> - parameter with code: '" + p.getCode() + "' and name: '" + p.getName() + "'",
...
/* 
 private int grawHGroup( GroupRequestItem grp, boolean exprFirst, int ind, Panel table, int level, CellRendererCallBack cb )
 {
  boolean groupFirst=true;
  for( RequestItem rri :  grp.getItems() )
  {
   if( rri instanceof ParameterRequestItem || rri instanceof ComplexFilteredRequestItem )
   {
    CellRendererInfo cri = cb.getCellRendererInfo(ind++, rri, grp.getItems().size()==grp.getDepth()?-1:grp.getDepth(), level, exprFirst, groupFirst);

    String cls = cri.getCellClass()+(exprFirst?" boldLeftBorder":"");
    
//    int depth = request.getRootGroup().getDepth();
//    
//    if( !exprFirst )
//    {
//     if( depth == -1 || depth == request.getRootGroup().getItems().size() )
//      cls+=" andBorder";
//     else if( depth == 1 )
//      cls+=" orBorder";
//    }
    ...
    ind=grawHGroup( (GroupRequestItem) rri, exprFirst, ind, table, level+1, cb );
 ...
 */
 ...
 private void renderHorizontalTable( Summary rep, final ReportRequest req, Panel table )
 {
  int collectionSumm = 0;

  addCell(table,"Collections","cellCollectionNameHeader hHeader");
  addCell(table,"Records<br/>in collection","cellCollectionCounterHeader boldLeftBorder hHeader");
  ...
  {
   String txt = references.get(i).getReqItem().getParameter().getCode()+"<sup>" + (i+1) + "</sup>";
   
   if( references.get(i).getReqItem() instanceof FilteredRequestItem )
    txt += "<br>(filtered)";
   
   addCell(table,txt,"cellParameterHeader hHeader"+(i==0?" boldLeftBorder":""));
  }
  
  final int[] summ = new int[references.size() + 1];

  addCell(table,"Result" + "<sup>" + (references.size() + 1) + "</sup>","cellResultHeader  boldLeftBorder hHeader");
  ...
  for(Summary br : rep.getRelatedCounters() )
  {
   ...
   addCell(table,br.getId() > 0 ? DataManager.getInstance().getCollection(br.getId()).getName() : "[All]","cellCollectionName");
   ...
   if( br.getId() >0 )
   {
    ...
    addCell(table, String.valueOf(br.getCount()), "cellCollectionCount boldLeftBorder counterCell", rr);
   }
   else
    addCell(table, String.valueOf(br.getCount()), "cellCollectionCount boldLeftBorder counterCell");
   ...
   for( int i=0; i < references.size(); i++ )
   {
    ...
    if( br.getRelatedCounters() != null )
    {
     for(Summary pc : br.getRelatedCounters())
     {
      ...
      {
       count = pc.getCount();
       withRels = pc.getRelatedCounters() != null && pc.getRelatedCounters().length > 0 ;
       break;
      }
     }
    }
...
//    rr.setAndOperation( request.isAndOperation() );
    ...
    String percent = br.getCount()==0?"":"<span class='percentage'>("+String.valueOf(count*100/br.getCount())+"%)</span>";
    
    addCell(table, String.valueOf(count)+(withRels?"<sup>+R</sup>":"")+percent, "cellParameterCount counterCell"+(i==0?" boldLeftBorder":""), rr);
   }
...
//   rr.setAndOperation( request.isAndOperation() );
   ...
   int resCnt = br.getRelatedCounters()!= null&&br.getRelatedCounters().length>0?br.getRelatedCounters()[0].getCount():0;
   
   String resStr = br.getCount()==0?"0":String.valueOf(resCnt)+"<span class='percentage'>("+String.valueOf(resCnt*100/br.getCount())+"%)</span>";
   
   addCell(table,resStr,"cellResultCount boldLeftBorder counterCell", rr);
  ...
  if( request.isCollectionSplit() && request.getCollections() != null )
  {
   ...
   addCell(table,String.valueOf(collectionSumm),"cellCollectionSummary boldLeftBorder hSummary counterCell",rr);
  }
  else
   addCell(table,String.valueOf(collectionSumm),"cellCollectionSummary boldLeftBorder hSummary counterCell");
  ...
  for( int i=0; i < references.size(); i++ )
  {
   ...
   addCell(table,String.valueOf(summ[i]),"cellParameterSummary hSummary counterCell"+(i==0?" boldLeftBorder":""),rr);
  }
  
  addCell(table,String.valueOf(summ[summ.length - 1]),"cellResultSummary boldLeftBorder hSummary counterCell",request);
  
  references.add( new ParamRef("<sup>" + (references.size() + 1) + "</sup> - " + expr, null ) );
 }
 
 /*
 private int drawVGroup(GroupRequestItem grp, Summary[] sLst, StringBuilder expr, boolean exprFirst, int ind, Panel table)
 {
  boolean groupFirst = true; 

  int op = grp.getDepth();
  
  if( op == grp.getItems().size() )
   op=-1;
  
  for( RequestItem ri : grp.getItems() )
  {
   if( ri instanceof GroupRequestItem )
    ind = drawVGroup( (GroupRequestItem) ri, sLst, expr, exprFirst, ind, table);
   else
   {
    Parameter p;
    
    if(ri instanceof ParameterRequestItem)
     p = ((ParameterRequestItem) ri).getParameter();
    else if(ri instanceof ComplexFilteredRequestItem)
     p = ((ComplexFilteredRequestItem) ri).getFilter().getParameter();
    else
     continue;
...
//      + (request.isAndOperation() ? " andTopBorder" : " orTopBorder");
    ...
    references.add( new ParamRef("<sup>" + ind + "</sup> - parameter with code: '" + p.getCode() + "' and name: '" + p.getName()+ "'",(ParameterRequestItem)ri) );

    addCell(table, p.getCode() + "<sup>" + (ind++) + "</sup>", "cellParameterHeader vHeader" + cls);
    ...
    for(Summary br : sLst)
    {
     ...
     String resStr = br.getCount()==0?"0":String.valueOf(count)+"<span class='percentage'>("+String.valueOf(count*100/br.getCount())+"%)</span>";
    ...
    addCell(table,String.valueOf(sum),"cellParameterSummary counterCell vSummary"+cls,rr);
 ...
 */
 ...
 private void renderVerticalTable( Summary rep, ReportRequest req, Panel table )
 {
  ...
  addCell(table,"Records<br/>in collection","cellCollectionCounterHeader  boldTopBorder vHeader");
  ...
  for(Summary br : rep.getRelatedCounters())
  {
   ...
   addCell(table,String.valueOf(br.getCount()),"cellCollectionCount  boldTopBorder counterCell",rr);
...
//   RequestItem rri = request.getRootGroup().getItems().get(0);
   if( request.isCollectionSplit() && request.getCollections() != null )
   {
    ...
    addCell(table, String.valueOf(sum), "cellCollectionSummary boldTopBorder vSummary counterCell", rr);
   }
   else
    addCell(table, String.valueOf(sum), "cellCollectionSummary boldTopBorder vSummary counterCell");
  }
  ...
  for( int i=0; i < references.size(); i++ )
  {
   ...
   addCell(table, pr.getReqItem().getParameter().getCode() + "<sup>" + (i+1) + "</sup>", "cellParameterHeader vHeader" + cls );
...
//  StringBuilder expr = new StringBuilder();
//  
//  int ind = drawVGroup(request.getRootGroup(), rep.getRelatedCounters(), expr, true, 1, table);
//  
//  references.add( new ParamRef("<sup>" + ind + "</sup> - "+expr, null ));
  
  addCell(table, "Result<sup>" + (references.size()+1) + "</sup>", "cellResultHeader boldTopBorder vHeader");
  sum=0;
  for(Summary br : rep.getRelatedCounters())
  {
   int count = br.getRelatedCounters()!=null&&br.getRelatedCounters().length>0?br.getRelatedCounters()[0].getCount():0;
   ...
   addCell(table, resStr, "cellResultCount boldTopBorder counterCell", rr);

   sum+=count;
  }
  
  addCell(table, String.valueOf(sum), "cellResultSummary boldTopBorder counterCell vSummary", request);
 ...
 private void addCell(Panel tbl, String text, String cls, final ReportRequest req )
 {
  Panel lp = new Panel();
  AdvTableLayoutData td = new AdvTableLayoutData();
 ...
 class ToolbarListener extends ButtonListenerAdapter
 {
  ...
  public void onClick(Button button, EventObject e)
  {
   ...
   else if( "EXPXL".equals(button.getStateId()) )
   {
    ...
    FileDownloader.downloadFile(GWT.getModuleBaseURL()+"reportExport?request="+reqStr+"&time="+System.currentTimeMillis());
   }
   else if( "EXPCSV".equals(button.getStateId()) )
   {
    String reqStr = request.toSerialString();
    
    FileDownloader.downloadFile(GWT.getModuleBaseURL()+"reportExport?format=csv&request="+reqStr+"&time="+System.currentTimeMillis());
   }
  }
 ...
 static class CellClickListener extends BaseItemListenerAdapter implements EventCallback
 {
  ...
  public void onClick(BaseItem item, EventObject e)
  {
   if(item.getStateId().equals("ids"))
   {
    ...
    {
     Window.open(GWT.getModuleBaseURL()+"dataExport?idsOnly=true&request="+req.toSerialString(), "_blank", "");
//     MessageBox.confirm("Confirm IDs download", "Do you want to download " + rc.getCount() + " IDs for this cell?",
...
//       });
   ...
   else if( item.getStateId().equals("records") )
   {
    ...
    {
      Window.open(GWT.getModuleBaseURL()+"dataExport?request="+req.toSerialString(), "_blank", "");
    }
    else
