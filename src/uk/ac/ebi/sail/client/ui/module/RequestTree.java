...
public class RequestTree extends Panel implements StudyCollectionStateListener {
	...
	private List<MakeReportListener> listeners = new ArrayList<MakeReportListener>(
			3);
	// private Store store;
	...
	public RequestTree() {
		...
		Collection<ExpressionRequestItem> pexpr = DataManager.getInstance()
				.getExpressions();
		...
		sm.addSelectionModelListener(new DefaultSelectionModelListenerAdapter() {
			public boolean doBeforeSelect(DefaultSelectionModel sm,
					TreeNode newNode, TreeNode oldNode) {
			...
			public void onSelectionChange(DefaultSelectionModel sm,
				...
				if (reqSelListener != null)
					reqSelListener.itemSelected(node == null ? null
							: (RequestItem) node.getUserObject());
			}
		...
		tree.addListener(new TreePanelListenerAdapter() {
			public boolean doBeforeNodeDrop(TreePanel treePanel,
				...
				JavaScriptObjectHelper.setAttribute(dragData.getJsObj(),
						"uk.ac.ebi.dd", false);
				...
				if (dragData instanceof GridDragData) {
					...
					if (recs.length == 1) {
						...
						if (dropObj instanceof Parameter)
							...
							addPredefinedQueryToGroup(
									(GroupRequestItem) dropObj, target);
					...
					Parameter p = (Parameter) ((TreeDragData) dragData)
			...
			public boolean onNodeDragOver(TreePanel treePanel, TreeNode target,
				...
				// ( (rri.getType() == Type.PARAMETER || rri.getType() ==
				// Type.FILTERED_SINGLE || rri.getType() ==
				// Type.FILTERED_COMPLEX) /*&& rri.getGroupItem() == null*/ ) )
		...
		new DropTarget(tree, cfg) {
			public boolean notifyDrop(DragSource source, EventObject e,
					DragData data) {
				// System.out.println("notifyDrop " + e.getTarget().getId() +
				// " dd: " + data.getJsObj());
				...
				if (data instanceof GridDragData) {
					...
					Parameter p = (Parameter) ((TreeDragData) data)
							.getTreeNode().getUserObject();
			...
			public String notifyEnter(DragSource source, EventObject e,
					DragData dragData) {
				JavaScriptObjectHelper.setAttribute(dragData.getJsObj(),
						"uk.ac.ebi.dd", true);
	...
	private ReportRequest prepareReportRequest() {
		...
		if (useRels.getValue()) {
			if (relsSpecific.getValue())
				rreq.setRelations(SelectRelationsDialog.getInstance()
						.getRelations());
		...
		if (useCollectionSplit.getValue()) {
			...
			if (splitBySpecific.getValue())
				rreq.setCollections(CollectionDialog.getInstance()
						.getCollections());
	...
	public void quickQuery(Parameter p) {
		...
		rreq.add(new ParameterRequestItem(p.getCode() + " (" + p.getName()
				+ ")", p));
  ...
  for(MakeReportListener l : listeners)
   l.makeReport(rreq);
	...
	private void addObjectToGroup(RequestItem rri, TreeNode sel) {
		if (sel == null)
			sel = ((DefaultSelectionModel) tree.getSelectionModel())
					.getSelectedNode();
		 ...
		 * 
		 * // TreeNode nGrn = new TreeNode(grr.getObjectName()); TreeNode nGrn =
		 * sel; nGrn.setLeaf(false);
		 * 
		 * nGrn.setText(grr.getObjectName());
		 * nGrn.setIconCls(grr.getIconClass()); nGrn.setUserObject(grr); //
		 * sel.getParentNode().replaceChild(nGrn,sel);
		 * 
		 * selRri.setGroupItem(grr); TreeNode nn = new
		 * TreeNode(selRri.getObjectName());
		 * nn.setIconCls(selRri.getIconClass()); nn.setUserObject(selRri);
	...
	public void addVariable(Variable v) {
		RequestItem rri = new PartRequestItem(v.getParameter().getCode() + "."
				+ v.getName() + " (" + v.getParameter().getName() + ")", v);

		addObject(rri);
	}

	public void addPredefinedQuery(GroupRequestItem g) {
		// ReportRequestItem rri = new ReportRequestItem(Type.PREDEF,
		// g.getGroupName(), g);
	...
	public void addParameter(Parameter p) {
		RequestItem rri = new ParameterRequestItem(p.getCode() + " ("
				+ p.getName() + ")", p);
	...
	public void addParameter(Qualifier q) {
		RequestItem rri = new PartRequestItem(q.getParameter().getCode() + "."
				+ q.getName() + " (" + q.getParameter().getName() + ")", q);

		addObject(rri);
	}

	public void addSplitFiltered(FilteredEnumeration fe) {
		RequestItem rri = new EnumFilteredRequestItem(fe.getParameter()
				.getCode() + " (" + fe.getParameter().getName() + ")", fe);

		addObject(rri);
	}

	public void addFiltered(ComplexFilter cf) {
		RequestItem rri = new ComplexFilteredRequestItem(cf.getParameter()
				.getCode() + " (" + cf.getParameter().getName() + ")", cf);
	...
	//
	// ReportRequestItem rri = new ReportRequestItem(Type.FILTERED_SINGLE,
	// p.getCode()+" ("+p.getName()+")", fe );
	//
	// addObject(rri);
	// }
	...
	//
	// ReportRequestItem rri = new ReportRequestItem(Type.FILTERED_COMPLEX,
	// p.getCode()+" ("+p.getName()+")", fel );
	//
	// addObject(rri);
	// }
	...
	//
	// if( fel.size() == 1 )
	// rri = new ReportRequestItem(Type.FILTERED_SINGLE,
	// p.getCode()+" ("+p.getName()+")", fel.get(0) );
	// else
	// rri = new ReportRequestItem(Type.FILTERED_COMPLEX,
	// p.getCode()+" ("+p.getName()+")", fel );
	//
	// addObjectToGroup(rri, null);
	// }
	...
	private void addParameterToGroup(Parameter p, TreeNode dropNode) {
		RequestItem prri = new ParameterRequestItem(p.getCode() + " ("
				+ p.getName() + ")", p);
	...
	public void addGroup(List<Parameter> lp) {
		...
		// tnode.setLeaf(false);
		// tnode.setUserObject(new
		// ReportRequestItem(Type.GROUP,"Group "+id,null));
		...
		for (Parameter pfl : lp) {
			...
			RequestItem nrri = new ParameterRequestItem(pfl.getCode() + " ("
					+ pfl.getName() + ")", pfl);
	...
	private class ReportBtListener extends ButtonListenerAdapter {
		...
		public void onClick(Button button, EventObject e) {
			...
			for (Node nd : tree.getRootNode().getChildNodes()) {
				...
				// ReportRequestItem rri = new ReportRequestItem( Type.GROUP,
				// null, gris);
				//
	...
	private class ReportEditor extends ButtonListenerAdapter {
		...
		public void onClick(Button button, EventObject e) {
			...
			if ("clear".equals(state)) {
				...
				TreeNode sel = ((DefaultSelectionModel) tree
				...
				// sel.getParentNode().equals( tree.getRootNode() ) )
				// sel = (TreeNode)sel.getParentNode();

				if (reqSelListener != null
						&& tree.getSelectionModel().isSelected(sel))
					reqSelListener.itemSelected(null);
	...
	static void removeNode(Node sel) {
		...
		if (sel.getNextSibling() != null) // There are another items in the
											// group. Just removing from group
		{
		...
		if (pNode.getChildNodes().length > 1) // It means last item in the group
												// where there are another items
		{
		...
		if (pNode.getPreviousSibling() != null) // Last and only item and there
												// are another groups below
		{
		...
		if (sibs[0].equals(nd)) {
			...
			for (Node mynd : sibs) {
				...
				if (((TreeNode) mynd).getUserObject() != null) {
					RequestItem rri = (RequestItem) ((TreeNode) mynd)
							.getUserObject();
	...
	private class NodeListener extends TreeNodeListenerAdapter {
		...
		public void onContextMenu(Node node, EventObject e) {
			...
			if (rItem instanceof GroupRequestItem) {
				...
				menu.addSeparator();
			} else if (rItem instanceof ParameterRequestItem
					|| rItem instanceof ComplexFilteredRequestItem) {
			...
			public void onClick(BaseItem item, EventObject e) {
				...
				if ("edit".equals(item.getStateId())) {
					if (mItm.getRI() instanceof GroupRequestItem) {
						MessageBox.prompt("Enter group name",
						...
						cf.setParameter((Parameter) ((ParameterRequestItem) mItm
								.getNode().getUserObject()).getParameter());

						ParameterFilterDialog fDlg = new ParameterFilterDialog(
								cf, false);

						fDlg.setListener(new ObjectAction<ComplexFilter>() {
							@Override
							public void doMultyAction(String actName,
									List<ComplexFilter> lp) {
							}
						...
						ComplexFilter cf = (ComplexFilter) ((ComplexFilteredRequestItem) mItm
								.getNode().getUserObject()).getFilter();
						...
						fDlg.setListener(new ObjectAction<ComplexFilter>() {
							...
							public void doAction(String actName, ComplexFilter p) {
								...
								if (p.isClean()) {
									ParameterRequestItem pri = new ParameterRequestItem(
											mItm.getRI().getName(), p
													.getParameter());
					...
					if (mItm.getNode().equals(sel))
						((DefaultSelectionModel) tree.getSelectionModel())
								.select(mItm.getNode());
	...
	public static String changeClass(String fromClass, String toClass,
			String classStr) {
