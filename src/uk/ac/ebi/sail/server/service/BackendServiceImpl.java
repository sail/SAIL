...
import uk.ac.ebi.sail.server.MyRemoteServiceServlet;
import uk.ac.ebi.sail.server.data.DataManager;

//import com.google.gwt.user.server.rpc.RemoteServiceServlet;

public class BackendServiceImpl extends MyRemoteServiceServlet implements
		BackendService {
	// SampleData data = new SampleData();

	public ParameterShadow updateParameter(ParameterShadow p)
	...
	public ParameterShadow addParameter(ParameterShadow p)
			throws ParameterManagementException {
	...
	public ClassifierShadow updateClassifier(ClassifierShadow p)
	...
	public Integer addCollection(CollectionShadow rs)
			throws CollectionManagementException {
		return DataManager.getInstance().addCollection(rs);
	}

	public void updateCollection(CollectionShadow p)
			throws CollectionManagementException {
	...
	public Map<String, String> getConfiguration() {
		return BackendConfigurationManager.getInstance()
				.getClientConfiguration();
	}

	@Override
	public int addStudy(StudyShadow studyShadow)
			throws StudyManagementException {
		return DataManager.getInstance().addStudy(studyShadow);
	}

	@Override
	public void updateStudy(StudyShadow studyShadow)
			throws StudyManagementException {
	...
	public Summary getCollectionSummary(int khID) {
		System.out.println("Collection summary request: "
	...
	public boolean sendMail(String msg, String pwd, String umail) {
		return DataManager.getInstance().sendMail(msg, pwd, umail);
	}

	public String[][] getUsersObject() {
		return DataManager.getInstance().getUsersObject();
	}

	public boolean delObject(int parseInt) {
		return DataManager.getInstance().delObject(parseInt);
	}

	public void addUser(String args) {
		DataManager.getInstance().addUser(args);
	}

	public void editUser(String args, String string) {
		DataManager.getInstance().editUser(args, string);
	}

	public User userMng(User user, String method, Map<String, String> loginData) {
		return DataManager.getInstance().userMng(user, method, loginData);
	}

	// joern
	public boolean delPara(Integer ID) {
		return DataManager.getInstance().delPara(ID);
	}

	@Override
	public void addUserToColl(String userId, String collectionId) {
		DataManager.getInstance().addUserToColl(userId, collectionId);

	}

	public void addMail(String mvalue, String method) {
		DataManager.getInstance().addMail(mvalue, method);

	}

	@Override
	public String[] getMailConf() {
		return DataManager.getInstance().getMailConf();

	}

	@Override
	public boolean changePwd(String oldPwd, String newPwd, User user) {
		return DataManager.getInstance().changePwd(oldPwd, newPwd, user);
